...
public class Charter implements Loggable
{
	...
	public static boolean to(LocationProperty locationProperty)
	{
		...
		final Map<LocationProperty, Location> charterLocations = getCharterLocations();
		final Location location = charterLocations.get(locationProperty);
		if (location == null)
		{
	...
	{
		return Interfaces.isInterfaceValid(CHARTER_INTERFACE_MASTER) || InteractionHelper.click(
			InteractionHelper.getRSNPC(Filters.NPCs.actionsEquals("Charter")), "Charter",
			() -> Interfaces.isInterfaceValid(CHARTER_INTERFACE_MASTER) ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE
		);
	}

	private static Map<LocationProperty, Location> getCharterLocations()
	{
		final Map<LocationProperty, Location> locations = new HashMap<>();
