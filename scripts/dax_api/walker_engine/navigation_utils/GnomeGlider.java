...
public class GnomeGlider
{
	...
	public static boolean to(Location location)
	{
		...
		final RSInterface option = InterfaceHelper.getAllInterfaces(GNOME_GLIDER_MASTER_INTERFACE).stream().filter(rsInterface -> {
			final String[] actions = rsInterface.getActions();
