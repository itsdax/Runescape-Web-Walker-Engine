...
public class NPCInteraction implements Loggable
{
	...
	public static boolean talkTo(Predicate<RSNPC> rsnpcPredicate, String[] talkOptions, String[] replyAnswers)
	{
		if (!clickNpcAndWaitChat(rsnpcPredicate, talkOptions))
		{
			return false;
		}
	...
	private static boolean clickNpcAndWaitChat(Predicate<RSNPC> rsnpcPredicate, String[] options)
	{
		return clickNpc(rsnpcPredicate, options) && waitForConversationWindow();
	}

	public static boolean clickNpc(Predicate<RSNPC> rsnpcPredicate, String[] options)
	{
		final RSNPC[] rsnpcs = NPCs.findNearest(rsnpcPredicate);
	...
	{
		while (WaitFor.condition(General.random(650, 800), () -> isConversationWindowUp() ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) == WaitFor.Return.SUCCESS)
		{
	...
	public static void handleConversation(String... options)
	{
		...
		while (limit++ < 25)
		{
			...
			final List<RSInterface> selectableOptions = getAllOptions(options);
	...
	{
		final List<RSInterface> list = getConversationDetails();
		if (list == null)
		{
			return null;
		}
		final Optional<RSInterface> optional = list.stream().filter(rsInterface -> rsInterface.getText().equals("Click here to continue")).findAny();
	...
	{
		final List<String> interfaces = getAllInterfaces().stream().map(RSInterface::getText).collect(Collectors.toList());
	...
	private static List<RSInterface> getConversationDetails()
	{
		...
		{
			final List<RSInterface> details = InterfaceHelper.getAllInterfaces(window).stream().filter(rsInterfaceChild ->
			{
				if (rsInterfaceChild.getTextureID() != -1)
				{
					return false;
				}
				final String text = rsInterfaceChild.getText();
	...
	{
		final ArrayList<RSInterface> interfaces = new ArrayList<>();
		for (int window : ALL_WINDOWS)
		{
